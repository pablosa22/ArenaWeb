...
namespace AppArenaWeb.Models
{
    public class AgendacModel
    {
        ...
        public List<AgendacModel> ListaAgendai()
        {
            ...
            if (Id != 0)
            {
                sql = $"SELECT I.ID, I.ID_PRODUTO, P.DESCRICAO, I.QTDE, I.PUNIT, I.VLDESCONTO FROM AGENDAI I, PRODUTO P WHERE ID_PRODUTO = P.ID AND I.ID_AGENDAC = '{Id}' ORDER BY P.DESCRICAO ASC";
            }
            DAL objDal = new DAL();
            DataTable dt = objDal.RetDataTable(sql);
            for (int i = 0; i < dt.Rows.Count; i++)
